...
class Maze:
    ...
    def breadth_first_solution(self):
        ...
        while len(queue) > 0:
            ...
            if current_element != self.target:
                ...
                current_element.unvisited_neighbours().reverse()
            ...
            if current_element != self.target:
                ...
                # otherwise pick a random unvisited neighbour, update its parent, set to visited, and add it to the stack
                    while len(possible_neighbors)>0:
                        next_element = possible_neighbors.pop(-1)
    ...
    def greedy_search(self):
        ...
        while len(sorted_list) > 0:
            ...
            # if you did not find the target
            if current_element != self.target:
                # for all unvisited neighbours, marks them as visited, set their parent and insert them onto the priority queue
                for next_element in current_element.unvisited_neighbours():
                    next_element.is_visited = True
                    next_element.set_parent(current_element)
                    bisect.insort(sorted_list, next_element) #__lt__ internally uses gscore
    ...
    def a_star_search(self):
        ...
        # while there are elements to explore
        while len(sorted_list) > 0:
            # the current element if the first element of the priority queue. mark it as visitie
            current_element = sorted_list.pop(0)
            ...
            if current_element != self.target:
                ...
                for next_element in current_element.unvisited_neighbours():
                    ...
                    if not next_element.is_seen:
                        ...
                        bisect.insort(sorted_list, next_element)  # __lt__ internally uses gscore
                    # otherwise
                        # check if the score is improved
                for element in sorted_list:
                    if element.score < current_element.score:
                        sorted_list.remove(element)
                        next_element.is_seen = True
                        element.update_fscore(element.fscore + 1)
